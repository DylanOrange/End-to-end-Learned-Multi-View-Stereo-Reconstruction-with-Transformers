...
def parse_args():
    ...
    parser.add_argument("--data_path", metavar="DIR",
                        help="path to raw dataset", default='/mnt/hdd/praktikum/Deep-Learning-in-Visual-Computing/scannet')
    parser.add_argument("--save_path", metavar="DIR",
        help="path to raw dataset", default='/mnt/hdd/praktikum/ldy/tsdf_gt')
    parser.add_argument("--save_name", metavar="DIR",
                        help="file name", default='all_tsdf_10')
...
def process_with_single_worker(args, scannet_files):
    for scene in tqdm(scannet_files):
        if os.path.exists(os.path.join(args.save_path, scene, 'full_tsdf_layer2.npz')):
            continue
...
def generate_pkl(args):
    ...
    split_path = '/mnt/hdd/praktikum/ldy/tools/split'
    fragment_path = '/mnt/hdd/praktikum/Deep-Learning-in-Visual-Computing/fragments-train'
    save_path = split_path

    all_scenes = sorted(os.listdir(allscene_path))
    ...
    for split in splits:
        fragments = []
        with open(os.path.join(split_path, 'scannetv2_{}.txt'.format(split))) as f:
            split_files = f.readlines()
        for scene in all_scenes:
            if 'scene' not in scene:
                continue
            if scene + '\n' in split_files:
                with open(os.path.join(fragment_path, scene, 'fragments_{}_15.pkl'.format(scene)), 'rb') as f:
